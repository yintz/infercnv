# Source code for testing (remove this when this is packaged).
source("infer_CNV.R")
...
matrix_one_long_2 <- matrix(c(1,2,4,7,9,11,12,14,17,19,16,14,13,11,10,7,6,4,3,1), ncol=1)
matrix_two <- matrix(1:10, ncol=2)
matrix_two_long <- matrix(1:40, ncol=2)
matrix_two_long_2 <- matrix(c(1,2,4,7,9,11,12,14,17,19,16,14,13,11,10,7,6,4,3,1,1,2,4,7,9,11,12,14,17,19,16,14,13,11,10,7,6,4,3,1), ncol=2)
matrix_three <- matrix(1:15, ncol=3)
matrix_five <- matrix(1:25, ncol=5)

context("Test average_over_reference")

avref_answer_1 <- avref_one
avref_answer_2 <- matrix(c((0,5), rep(5,5)), ncol=2)
...
avref_answer_4 <- matrix(c(rep(-12.5,5),
                           ...
                           rep(2.5),
                           rep(7.5)), ncol=5)
avref_answer_5 <- avref_zeros

test_that("average_over_reference works with one observation, one reference",{
    expect_equal(average_over_reference(average_data=matrix_one,
                                        reference_observations=c(1)1),
                 avref_answer_1)})
test_that("average_over_reference works with two observations, one reference",{
    expect_equal(average_over_reference(average_data=matrix_two,
                                        reference_observations=c(1)),
                 avref_answer_2)})
test_that("average_over_reference works with three observations, two reference",{
    expect_equal(average_over_reference(average_data=matrix_three,
                                        reference_observations=c(1,3)),
                 avref_answer_3)})
test_that("average_over_reference works with five observations, two reference",{
    expect_equal(average_over_reference(average_data=matrix_five,
                                        reference_observations=c(2,5)),
                 avref_answer_4)})
test_that("average_over_reference works with one observation (all zeros), one reference",{
    expect_equal(average_over_reference(average_data=matrix_zeros,
                                        reference_observations=c(1)),
                 avref_answer_5)})
...
center_answer_3 <- matrix(c(rep(-6,5),rep(-5,5),rep(0,5),rep(5,5),rep(6,5)), ncol=5)
center_answer_4 <- matrix(rep(0,25), ncol=5)

test_that("center_with_threshold works with one observation, threshold too large to affect",{
    expect_equal(center_with_threshold(center_data=matrix_one,
                                       threshold=100),
                 center_answer_1)})
test_that("center_with_threshold works with three observations, threshold too large to affect",{
    expect_equal(center_with_threshold(center_data=matrix_three,
                                       threshold=100),
                 center_answer_2)})
test_that("center_with_threshold works with five observations, threshold affecting some",{
    expect_equal(center_with_threshold(center_data=matrix_five,
                                       threshold=6),
                 center_answer_3)})
test_that("center_with_threshold works with one observation, threshold of 0, affecting all",{
    expect_equal(center_with_threshold(center_data=matrix_five,
                                       threshold=0),
                 center_answer_4)})
...
test_that("above_cutoff works with one observation, cutoff too large to affect",{
    expect_equal(above_cutoff(data=matrix_one,
                              cutoff=0),
                 above_answer_1)})
test_that("above_cutoff works with three observations, threshold too large to affect",{
    expect_equal(above_cutoff(data=matrix_three,
                              cutoff=0),
                 above_answer_2)})
test_that("above_cutoff works with one observation, threshold excluding two.",{
    expect_equal(above_cutoff(data=matrix_one,
                              cutoff=2),
                 above_answer_3)})
test_that("above_cutoff works with three observations, threshold excluding three.",{
    expect_equal(above_cutoff(data=matrix_three,
                              cutoff=12),
                 above_answer_4)})
test_that("above_cutoff works with one observation, threshold excluding all.",{
    expect_equal(above_cutoff(data=matrix_one,
                              cutoff=100),
                 above_answer_5)})
test_that("above_cutoff works with three observations, threshold excluding all.",{
    expect_equal(above_cutoff(data=matrix_three,
                              cutoff=100),
                 above_answer_6)})
...
test_that("remove_noise works with one observation, one ref, threshold 0",{
    expect_equal(remove_noise(ref=1,
                              ...
                              threshold=0),
                  noise_answer_1)})
test_that("remove_noise works with one observation, one ref, threshold removing some",{
    expect_equal(remove_noise(ref=1,
                              smooth_matrix=matrix_one,
                              threshold=4),
                  noise_answer_2)})
test_that("remove_noise works with one observation, one ref, threshold removing all",{
    expect_equal(remove_noise(ref=1,
                              smooth_matrix=matrix_one,
                              threshold=6),
                  noise_answer_3)})
...
tail_answer_4 <- matrix(c(1:4,rep(0,5),10,rep(0,5),16:24,rep(0,5),30,rep(0,5),36:40),ncol=2)
tail_answer_5 <- matrix_zeros

test_that("remove tails works with one contig, one observation, no tail length",{
...
test_that("remove tails works with 3 contig, two observations, tail length 5",{
    expect_equal(remove_tails(smooth_matrix=matrix_two_long,
                              ...
                              tail_length=5),
                 tail_answer_4)})
test_that("remove tails works with one contig, one observation, tail length longer than contig",{
    expect_equal(remove_tails(smooth_matrix=matrix_one,
                              chr=1:length(matrix_one),
                              tail_length=100),
                 tail_answer_5)})
...
test_that("smooth_window works with two observations, window_length 5",{
    expect_equal(round(smooth_window(data=matrix_two_long_2,
                                     window_length=5),2),
                 smooth_answer_4)})
test_that("smooth_window works with one observation, window_length longer than measurements",{
    expect_equal(smooth_window(data=matrix_one,
                               window_length=100),
                 smooth_answer_5)})
