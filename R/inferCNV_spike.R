...
.get_hspike_obs_mean_var_trend <- function(infercnv_obj, plot=F) {
    ...
    for (cnv_level in names(cnv_level_to_mean_sd) ) {
        ...
        fit = lm(log(num_cells) ~ log(sd_vals)) #note, hbadger does something similar, but not for the hmm cnv state levels
